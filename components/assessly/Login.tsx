...
/** Static sign-in. There's no auth backend, so any submit enters the workspace
 *  (and the choice is remembered in localStorage by the store). */
export function Login() {
  const { signIn } = useAssessly();
    ...
    <div style={{ height: "100vh", display: "flex", background: "#fff", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 32px", background: "#F4F6F9", overflowY: "auto" }}>
        <div style={{ width: "100%", maxWidth: 392 }}>
          <h2 style={{ fontSize: 27, fontWeight: 800, color: C.navy, letterSpacing: "-.5px", margin: 0 }}>Sign in</h2>
          <p style={{ color: C.muted, fontSize: 14.5, margin: "9px 0 32px", lineHeight: 1.45 }}>Use your Mercer | Mettl work account to continue.</p>
