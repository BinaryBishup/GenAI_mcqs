...
/** Shown when a user is signed in on the shared bootstrap password: they must
 *  choose a private password (enter + confirm) before reaching the workspace. */
export function SetPassword() {
  const { user, setPassword, signOut } = useAssessly();
  ...
  const submit = async () => {
    ...
    // on success the store clears mustReset and the Shell swaps to the workspace.
  };
    ...
    <div style={{ height: "100vh", display: "flex", background: "#fff", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 32px", background: "#F4F6F9", overflowY: "auto" }}>
        <div style={{ width: "100%", maxWidth: 392 }}>
          <h2 style={{ fontSize: 27, fontWeight: 800, color: C.navy, letterSpacing: "-.5px", margin: 0 }}>Set your password</h2>
          <p style={{ color: C.muted, fontSize: 14.5, margin: "9px 0 32px", lineHeight: 1.45 }}>
            Your account is on a temporary shared password. Choose a private one
            for <strong style={{ color: C.slate }}>{user?.email}</strong> to continue.
          </p>

          <label style={{ fontSize: 13, fontWeight: 600, color: C.slate, display: "block", marginBottom: 8 }}>New password</label>
          <HInput
            type="password"
            value={pw}
          ...
          />
          <label style={{ fontSize: 13, fontWeight: 600, color: C.slate, display: "block", marginBottom: 8 }}>Confirm new password</label>
          <HInput
            type="password"
            value={confirm}
          ...
          <div style={{ textAlign: "center", marginTop: 18 }}>
            <span onClick={signOut} style={{ fontSize: 13, color: C.muted, fontWeight: 600, cursor: "pointer" }}>
              Sign out
            </span>
          </div>
        </div>
