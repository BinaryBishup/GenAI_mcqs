...
export interface AuthUser {
  ...
  teams: Team[];
  /** True while the account is on the shared bootstrap password and must set a
   *  private one before reaching the workspace (must_reset_password metadata). */
  mustReset: boolean;
}
...
function userFromSession(session: Session | null): AuthUser | null {
  ...
  if (!u) return null;
  const meta = (u.user_metadata ?? {}) as { full_name?: string; team?: string; teams?: string[]; must_reset_password?: boolean };
  const teams: Team[] =
    meta.team === "ALL"
      ? [...TEAMS]
      : [...new Set([meta.team, ...(meta.teams ?? [])])].filter((t): t is Team => TEAMS.includes(t as Team));
  return {
    name: meta.full_name || u.email?.split("@")[0] || "Member",
    email: u.email ?? "",
    team: (meta.team as TeamMeta) ?? "HACK",
    teams: teams.length ? teams : ["HACK"],
    mustReset: meta.must_reset_password === true,
  };
}
...
interface Ctx {
  ...
  /** Set a new private password and clear the must-reset flag (forced-reset flow). */
  setPassword: (newPassword: string) => Promise<{ error?: string }>;
  signOut: () => void;
...
export function AssesslyProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supa.auth.getSession().then(({ data }) => {
      ...
      setUser(u);
      // Land on whatever URL the user opened (deep links work); login screen
      // keeps the URL so the destination survives the sign-in.
    ...
    const { data: sub } = supa.auth.onAuthStateChange((_event, session) => {
      ...
      if (u) {
        setScreen((prev) => {
          if (prev === "login") {
            // continue to the deep-linked page the user originally opened
            applyRoute(parsePath(window.location.pathname));
            return parsePath(window.location.pathname).screen;
          }
          return prev;
        });
      } else {
  ...
  const setPassword = useCallback(async (newPassword: string) => {
    // The user's own session can update both password and their user_metadata in
    // one call, so clearing the flag needs no admin route.
    const { error } = await supabaseBrowser().auth.updateUser({
      password: newPassword,
      data: { must_reset_password: false },
    });
    if (error) return { error: error.message };
    setUser((prev) => (prev ? { ...prev, mustReset: false } : prev));
    return {};
  }, []);
