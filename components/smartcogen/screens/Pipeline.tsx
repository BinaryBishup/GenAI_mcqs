...
import { isOngoing, useSmartCoGen } from "../store";
...
function useBuckets() {
  const { runs, finalisedIds } = useSmartCoGen();
...
export function Dashboard() {
  const { go, openReview, openFinalised } = useSmartCoGen();
...
export function Ongoing() {
  ...
  const { openReview } = useSmartCoGen();
  return (
...
export function Finalised() {
  const { openFinalised } = useSmartCoGen();
