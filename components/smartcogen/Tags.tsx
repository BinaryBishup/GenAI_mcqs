...
export function TagModal({ itemType, itemId, onClose }: { itemType?: TagItemType; itemId?: string; onClose: () => void }) {
  const { tags, toggleTagItem, createTag, openTag, toast } = useSmartCoGen();
...
export function TagScreen() {
  const { activeTagId, tags, runs, catalog, openReview, openFinalised, openBank, toggleTagItem, finalisedIds } = useSmartCoGen();
