...
import { loadSeedPool, planSeeds, type SeedRow, type SeedAssignment } from "@/lib/pipeline/seed-plan";
import { generateDiagram } from "@/lib/ai/diagram";
import { checkPlag } from "@/lib/pipeline/plag";
import { checkAnswer, arbitrate } from "@/lib/ai/answer-check";
import { embedTexts, maxCosine, cosine } from "@/lib/ai/embed";
...
import { shortId, normalizeText } from "@/lib/utils";
 ...
 * Persists everything to Postgres and streams events through `emit`.
 */
export async function runWorkflow(req: GenerateRequest, emit: Emit): Promise<{ runId: string; mcqs: MCQ[] }> {
  ...
  // Track in-flight run_events inserts so we can drain them before the request
  // handler returns — otherwise a cancelled response can cut the writes short.
  const pendingWrites: Promise<unknown>[] = [];
  const log = (evt: StreamEvent) => emitAndLog(emit, db, runId, evt, pendingWrites);
  ...
  try {
    ...
    await bestEffortUpdate(db, "runs", runId, {
    ...
    const accepted = await uniquenessPass(mcqs, seedByIndex, sourceQuestions, req2, model, log, db, runId, sourceRef);

    // ---- 5. Review pass (correctness / uniqueness / quality) ---------------
    await db.from("runs").update({ status: "reviewing" }).eq("id", runId);
    log({ type: "phase", data: { phase: "review", message: `Reviewing ${mcqs.length} questions for correctness & quality...` } });
    await reviewBatch(mcqs, sourceQuestions, req2, model, log, db, runId, undefined, seedByIndex);

    // ---- 5b. Regenerate anything the review rejected, re-check, re-review ---
    await regenerateRejects(mcqs, seedByIndex, accepted, sourceQuestions, req2, model, log, db, runId);
    ...
    await verifyPass(mcqs, seedByIndex, sourceQuestions, req2, model, log, db, runId);

    // ---- 5d. Diagram-as-code images (best-effort) --------------------------
    if (req2.create_images) {
      await generateImages(mcqs, req2, model, log, db, runId);
    }
    ...
    await db.from("runs").update({ status: "error", error_message: message, finished_at: new Date().toISOString() }).eq("id", runId);
    ...
    chunks.map(async (indices) => {
      try {
        ...
        const msg = await llm().complete({
          model,
          maxTokens: Math.min(16000, Math.max(2000, indices.length * 700 + 800)),
...
async function persistContent(db: Database, runId: string, index: number, mcq: MCQ) {
  await db
    .from("mcqs")
    .update({
      question: mcq.question,
...
async function persistPlag(db: Database, runId: string, index: number, mcq: MCQ) {
  await db
    .from("mcqs")
    .update({ plag_status: mcq.plag_status, plag_matches: mcq.plag_matches, plag_attempts: mcq.plag_attempts })
    .eq("run_id", runId)
    .eq("index", index);
  await bestEffortUpdate(db, "mcqs", null, { diversity_status: mcq.diversity_status ?? "ok" }, { run_id: runId, index });
}
...
async function uniquenessPass(
  ...
  await db.from("runs").update({ status: "plagchecking" }).eq("id", runId);
  ...
  for (let i = 0; i < mcqs.length; i++) {
    ...
    while (true) {
      ...
      if (!tooSimilar && !plagged) {
        ...
        await persistPlag(db, runId, i, mcqs[i]);
      ...
      if (fresh.length) {
        ...
        mcqs[i].plag_attempts = attempt;
        await persistContent(db, runId, i, mcqs[i]);
        if (semantic) {
...
async function regenerateRejects(
  ...
  for (const i of rejects) {
    ...
    await reviewBatch(mcqs, sourceQuestions, req, model, log, db, runId, [i], seedByIndex);
  }
}
  ...
  async function worker() {
    while (true) {
      ...
      if (svg) {
        ...
        await bestEffortUpdate(db, "mcqs", null, { image_svg: svg }, { run_id: runId, index: i });
      }
    }
  }
...
async function verifyPass(
  ...
  await db.from("runs").update({ status: "verifying" }).eq("id", runId);
  ...
  const worker = async () => {
    while (true) {
      ...
      await bestEffortUpdate(db, "mcqs", null, { answer_check_status: r.status, answer_check_index: r.index, answer_check_notes: r.notes }, { run_id: runId, index: i });
...
async function loadSamples(req: GenerateRequest) {
  ...
  for (const f of req.sample_files) {
    const rows = await fetchPerFile(db, f, req.difficulty, visible);
    all.push(...rows);
  }
  return all;
}
 ...
 * minutes, blowing past maxDuration and leaving the UI stuck on
...
async function streamMcqArray(model: string, userPrompt: string, maxTokens: number): Promise<any[]> {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    // Streaming was only ever used to await the final message, so a plain
    // completion is equivalent. SYSTEM_INSTRUCTIONS is the large stable prefix
    // worth caching on providers that support it.
    const msg = await llm().complete({
      model,
      maxTokens,
    ...
    try {
      ...
      const usage = `in=${msg.inputTokens} out=${msg.outputTokens}`;
...
async function insertMCQs(runId: string, mcqs: MCQ[]) {
  ...
  const { error } = await db.from("mcqs").insert(rows);
...
function emitAndLog(
  ...
  emit(evt);
  const p = db.from("run_events").insert({ run_id: runId, type: evt.type, data: evt.data });
  // Push into the caller's pending list so they can drain before the request
  // handler returns, rather than being cut short with the writes unpersisted.
